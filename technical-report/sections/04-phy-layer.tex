...
\paragraph{Stationary-carrier excision.}
\label{sec:excision}
A CW carrier at the frame's own power defeats every stage at once: it
leaks through the rectangular detection window into a dozen bins and
lands in some mask shift's tone bins, it adds a correlation floor of
$\sim$0.2 times its amplitude ratio to the ZC stage and dominates that
stage's energy normalisation, and on its subcarrier it sits $\sim$17\,dB
above the per-carrier signal.  Measured (\S\ref{sec:interference}) the
detector is blind at ISR $+10$\,dB and the demodulator fails from ISR
$-7$\,dB, while an ideal 30-Hz notch at the carrier's frequency decodes
15 of 15 at every ratio up to $+10$\,dB.  So the receiver finds such
carriers and notches them out of the \emph{samples} before the Hilbert
transform, in all three implementations with one definition.  Per
detection block the in-band bins above $4\times$ the block's mean power
are its excess bins (at most four; the mean rather than the median,
because a carrier alone on a quiet channel has a median of zero and
then every crumb of leakage counts; $4\times$ because a carrier halfway
between two bins keeps only 40\,\% of its power in each).  A bin in
excess in at least 85\,\% of the blocks -- the whole recording frame at
once, the last two tone windows streaming -- is a carrier: the
receiver's own comb is on for one tone field, a stranger's preamble
train lights each bin exactly two thirds of the time, and white noise
never repeats a bin.  The carrier's frequency inside the bin is the
phase advance of that bin between consecutive blocks, unambiguous over
half a bin, taken at the strongest bin of an adjacent group (the
closest one; the far one wraps).  A second-order IIR notch 30\,Hz wide
($r = 0.9921$, Q14 coefficients, int16-saturated output) removes it.
The streaming receiver keeps one notch bank in front of the shared
sample ring, run once per sample index whichever instance pushes first,
and releases a notch by its own monitor once the rejected power stays
under 1/32 of the input, or under 4 LSB rms, for four 4096-sample
ticks (shorter ticks let white noise trip the ratio; without the
absolute floor the notch's ringing on a silent input held it forever).
A rest-band trim inside the tone metric was tried first and removed: it
moved a noisy golden vector's coarse shift and bought nothing.
