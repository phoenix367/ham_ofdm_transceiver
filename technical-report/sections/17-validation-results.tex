...
instead of being blind.  At EXTREME the carrier column is partial: the
float receiver is back under 15\,\% PER from ISR $+3$\,dB, the
streaming receiver at 38--63\,\%, because a $-15$\,dB frame leaves the
carrier's frequency estimate noisier and the streaming finder
estimates over one tone window where the float model has the whole
recording.  Voice is unchanged ($-6$ to $-10$\,dB): its harmonics fall
on most carriers and its syllables on half the symbols, so the
weighting has no clean reference, and the detector's lock goes at ISR
0\,dB.  A frequency-tracking loop on the notch's rejected component is
the open thread.  The firmware with all four mechanisms runs on the
