...
cycle this low that looks impossible, and chasing it produced a wrong
answer before the right one.  Both are reported, because the way the
wrong one survived two rounds of measurement is the more useful part.
...
Across all 69 transmissions in this campaign, 2376 frames were keyed
and 18 were lost: a \textbf{group-loss rate of 0.76\,\%}.  That is not
a defect, it is the acquisition statistics of non-acknowledged
broadcast, and \S\ref{sec:cport_bcast} already records the same
phenomenon from the other direction --- 38 of 39 groups delivered
across an earlier campaign at rung~4, a 2.6\,\% loss rate.  The voice
path at rung~12 is running three times better than the documented
figure, and the honest conclusion is that it was never misbehaving.

Preamble acquisition is a threshold decision.  A high signal-to-noise
ratio makes a miss unlikely; it does not make it impossible, and
nothing in a broadcast retries.  Every acknowledged path on this radio
hides exactly this by retransmitting --- which is precisely why the
counters \texttt{frames\_ok} and \texttt{frames\_lost} exist, and why
they were built into the broadcast EOS in the first place.  The
instinct that a clean channel should lose nothing is right about the
\emph{channel} and wrong about the acquisition in front of it.
...
the missing narrative, and two rounds of measurement confirmed a story
that a third round demolished.  What broke the tie was not more data of
the same kind --- it was changing the measurement so it could not
flatter the hypothesis (interleaving the arms) and recording the raw
artefact rather than a statistic derived from it (the byte streams).
Both were cheap, and neither was done first.
...
This property is load-bearing rather than a nicety, and the previous
paragraphs are why.  A 0.76\,\% group loss is not a bug awaiting a
fix; it is what non-acknowledged broadcast costs, and a voice mode has
to be built to survive it rather than to avoid it.  Byte-aligned groups
are what convert an unavoidable acquisition miss into a missing
syllable.
