...
This subsection reports an evaluation, not an implementation.  Nothing
in \texttt{cport/} changed, and the conclusion is that nothing should
--- but the measurements below decide what a voice mode would cost and
which of its problems are fixable.
...
Algorithmic latency is the chunk length plus its \emph{right} context.
Left context is audio the encoder already has, and is free; only
lookahead delays the output.  Separating the two turns a 7.3\,s pipeline
into a 1.3\,s one for two tenths of a decibel.
...
narration --- LibriVox, public domain, one speaker --- gives the
end-to-end picture for both languages.
...
The bitrate fits, the compute fits, and the transport needs no change:
the tokens are opaque bytes of exactly the size the streaming burst
machinery already moves, and enrolment is two messages.  A voice mode
would be a host program, like KISS and the IP tunnel, and
\texttt{cport/} would not change.  It also need not be
store-and-forward: \S\ref{sec:voice_latency} settles the codec at
1.32\,s of algorithmic latency with flat memory, which puts it an order
of magnitude below the link's own turnaround and makes continuous
operation the natural mode rather than an aspiration.
