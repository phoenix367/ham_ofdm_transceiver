...
\begin{table}[H]
    ...
    \midrule
    Shared raw \texttt{int16} ring & 288\,KB & EXTREME re-anchor
                                               lookback (124\,478) \\
    Scratch arena (3 phases unioned) & 129\,KB & EXTREME ZC geometry \\
    Tone summaries, 3 instances & 94\,KB & each instance's own mode \\
    LLR buffers (\texttt{int32}) & 24\,KB & 192\,KB with EXT frames \\
    Streaming transmitter & 10\,KB & one IFFT tile \\
    Everything else & 154\,KB & --- \\
    \midrule
    \textbf{Total, three modes} & \textbf{699\,KB} & 929\,KB with EXT
                                                     frames \\
    Transmit-only image & 28\,KB & \\
    \bottomrule
    \end{tabular}
\end{table}
...
EXTREME count, and assumed a build without EXT frames. Both are now
addressed, and the remaining floor --- 288\,KB ring $+$ 129\,KB arena
$+$ 94\,KB summaries $=$ 511\,KB --- still exceeds the part. The
conclusion is an H743-class device, not the H723xG.
...
by a property of the waveform. The arena is also visibly balanced --- its
three phases sit within 6\,kB of each other --- which is the signal that
the union has no slack left: it costs the maximum, so no single phase
can be shrunk profitably alone.
