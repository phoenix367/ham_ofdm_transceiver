...
\subsubsection*{The rung nobody negotiates}

A broadcast has no peer to negotiate with, which makes the rung a
choice rather than a measurement. The default is the rung the link last
used, floored at rung~4 --- leaving it unspecified does not mean ``take
four minutes''. But an explicit rung is honoured exactly as given, and
that matters more than it looks: which modes a board is \emph{listening
on} follows the negotiated rung, and the only mode an idle station is
guaranteed to still be receiving is EXTREME
(Section~\ref{sec:cport_radio}). A beacon meant for stations that have
never been heard from belongs there, and the air-time cap is what makes
it affordable --- at rung~0 a group collapses to one 42-second frame
instead of four.

The measurement is the case that matters: with board~B freshly booted
and having never exchanged a frame with~A --- listening on EXTREME
alone --- a \texttt{bcast -r 0} from~A arrived byte-exact at
$+16.2$\,dB, in two groups of one frame each. At rung~4 and a 122-byte
payload (two groups per broadcast, four frames then two), a
twelve-broadcast run delivered \textbf{24 of 24 groups}, 72 frames, and
reported zero losses at $+24.7$\,dB; across the whole campaign 38 of 39
groups arrived.
