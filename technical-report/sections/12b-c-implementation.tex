...
station decodes to a \texttt{.wav}.  It works, and the three defects it
took to get there are more instructive than the fact that it does.
...
\paragraph{Frame loss on a clean wire, and what it actually was.}
The transmissions kept reporting losses --- 7.7\,\%, 11.8\,\%,
14.4\,\% of bytes --- on a cross-wired pair at rung~12 carrying
250\,bit/s.  On a channel with tens of decibels of margin and a
duty cycle this low, that is not a channel problem, and it was not.

A lost unit is always the same size: \textbf{one group, 30 bytes} ---
a SYNC frame plus its partner, 0.96\,s of speech.  Three independent
measurements agree on it.  The recording comes back 0.96\,s short; the
byte streams differ by exactly 30; and the sent/received dump names the
offset outright:

\begin{quote}\ttfamily\small
sent 660 B, received 630 B, missing 30 B\\
streams agree for the first 30 B\\
$\Rightarrow$ 30 B missing at offset 30\\
\hphantom{$\Rightarrow$} resumes at sent-offset 60; tail after that matches
\end{quote}
...
and the loss is discovered only from the sequence gap in the
\emph{next} SYNC\@.  A silent failure of this shape is exactly what the
sequence field exists for.
...
nothing.  The receiving board's capture-FIFO overrun counter
(\texttt{cap\_overruns}) stayed at zero across every losing run --- the
board was not dropping samples.  Its \texttt{bc\_rx\_lost} beacon
counter, which only the failed-block path increments, also stayed at
zero --- the frames were never \emph{heard}, not mis-decoded.  And the
mid-stream broadcast statistics exchange, which does make the receiver
key a frame, showed no correlation: three of four losses occurred in
runs that had none.

Both real causes were on the host, and both are ordering.

The first is a race.  The web server is threaded and the browser posted
audio blocks fire-and-forget from \texttt{onaudioprocess}, so the
encoder and the broadcast sender ran on several threads at once ---
\textbf{27 collisions in a single 20\,s transmission}, counted.  Two of
them corrupt the token stream directly, but the damaging one is the
broadcast-open flag, which was read \emph{before} the USB write and
assigned \emph{after} it: two threads both see it clear and both send a
non-continuation command, and the firmware treats the second as a new
broadcast --- closing the one on the air, overwriting the unsent bytes
and resetting the sequence to zero.  Serializing the ingest fixes it,
but a server-side lock alone does not order the \emph{wire}: two
concurrent \texttt{fetch()} calls may land in either order, so the
browser chains its posts and drains the chain before the close.

The second is the warm-up, and it is the interesting one.  The link
must be warmed before each transmission --- the rate ladder decays
after \texttt{RX\_STALE\_S} of silence and an idle station falls back to
listening on EXTREME only, so a cold broadcast goes out at a rung the
peer may not be receiving on.  The warm-up is an ARQ exchange, and it
was declared complete the moment the far station \emph{decoded} the
message.  But the acknowledgement it owed was still to be keyed, and
the link is half duplex: the broadcast then started into a receiver
that was transmitting, and a group went whole.  Waiting for the
transmit board's \texttt{pending} to clear and its queues to drain
costs about a second, and costs it while the operator has not started
talking.

\begin{table}[H]
    \centering
    \caption{Group loss against the warm-up settle.  Same two boards,
             same 20\,s of audio, rung~12, no other change.  A loss is
             always exactly one group.}
    \label{tab:voice_settle}
    \small
    \begin{tabular}{@{}lrrr@{}}
    \toprule
    \textbf{Warm-up} & \textbf{Transmissions} & \textbf{Lost a group} &
    \textbf{Rate} \\
    \midrule
    returns on decode (as found) & 23 & 5 & 21.7\,\% \\
    \textbf{waits for the exchange} & \textbf{16} & \textbf{0} &
        \textbf{0\,\%} \\
    \bottomrule
    \end{tabular}
\end{table}

Fisher's exact test gives $p = 0.066$ two-tailed, and the probability of
16 consecutive clean transmissions at the unsettled rate is $0.020$.
That is supporting evidence, not proof, and one detail is unexplained:
of the four losses whose position could be recovered, three sat near the
head of the stream where the warm-up argument places them and one sat
late, where it does not.  Every recording is therefore written with its
sent and received byte streams beside it, so the next occurrence names
itself instead of being inferred from a duration.
