...
\emph{A test configuration can hide a real memory defect.}  The
out-of-bounds read has existed for as long as the burst path, and every
suite runs under AddressSanitizer.  It was invisible because the tests
build with the default \texttt{ST\_MSG\_MAX} of 256, where
\texttt{pool[-1]} lands \emph{inside} the neighbouring controller
struct: contained undefined behaviour that a sanitizer cannot see.  At
the shipping 3328 and 4096 the same expression reads about 3\,kB outside
the object.  Sanitizers check the configuration you give them, and the
configuration under test was not the one that ships.
